...
export default function EditarEmpresa() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          nombre_impreso: data.nombre_impreso,
          logo: null,
          logo_impreso: null,
          pais_id: data.pais_id ?? 0,
          moneda_id: data.moneda_id ?? 0,
          correo: data.correo,
          telefono: data.telefono,
          nfi: data.nfi,
          prefijo_tarjetas: data.prefijo_tarjetas,
        });
      } else {
        console.error('Error al obtener la empresa:', response.status, response.statusText);
        setErrorMessage(`Error al obtener la empresa: ${response.status} ${response.statusText}`);
      }
    } catch (error) {
      console.error('Error en la solicitud:', error);
      setErrorMessage('Error al obtener la empresa');
    }
  }, [id]);
  ...
  useEffect(() => {
    const loadData = async () => {
      await Promise.all([fetchEmpresa(), fetchPaises(), fetchMonedas()]);
    };
    loadData();
  }, [fetchEmpresa, fetchPaises, fetchMonedas]);
        ...
        <div className="flex-1 flex flex-col">
          <main className="flex-1 p-8">
            <div className="max-w-4xl mx-auto">
              ...
              <form onSubmit={handleSubmitEditar} className="bg-white p-6 rounded-lg shadow-xl border">
