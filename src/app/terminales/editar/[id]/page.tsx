...
export default function EditarTerminal() {
  ...
  // Cargar empresas, centros de costos y datos de la terminal
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [empresasRes, costosRes, terminalRes] = await Promise.all([
          fetch('/api/empresas'),
          fetch('/api/costos'),
          fetch(`/api/terminales/${id}`),
        ]);

        if (!empresasRes.ok || !costosRes.ok || !terminalRes.ok) {
        ...
        setFormData({
          id: terminalData.id,
          empresa: terminalData.empresa || '',
          estacion_servicio: terminalData.estacion_servicio || '',
          codigo_terminal: terminalData.codigo_terminal || '',
          nombre_terminal: terminalData.nombre_terminal || '',
          numero_serie: terminalData.numero_serie || '',
          mac: terminalData.mac || '',
          modelo: terminalData.modelo || '',
          marca: terminalData.marca || '',
        });
  ...
  const filteredCostos = formData.empresa
    ...
    : [];
  console.log('FormData.empresa:', formData.empresa);
  console.log('FilteredCostos:', filteredCostos);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Terminal actualizada con éxito');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 py-4 px-3 sm:px-4 lg:px-6">
      ...
      <div className="w-full max-w-2xl sm:max-w-5xl bg-white border border-gray-300 p-6 sm:p-10 mx-auto">
        <h1 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-10 text-center">Editar Terminal</h1>
